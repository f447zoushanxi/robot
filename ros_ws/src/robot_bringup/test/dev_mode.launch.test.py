...
class TestDevModeBringup(unittest.TestCase):
    ...
    def _spin_until(self, condition, timeout_sec=20.0):
    ...
    def test_core_nodes_started(self):
        self.assertTrue(self._spin_until(lambda: len(self.node.get_node_names()) > 1, timeout_sec=20.0))
    ...
    def test_voice_to_nlu_and_executor_progress(self):
        msg = String()
        msg.data = '请给我递水'
        self.assertTrue(self._spin_until(lambda: self.voice_pub.get_subscription_count() > 0, timeout_sec=20.0))
        for _ in range(5):
            self.voice_pub.publish(msg)
            rclpy.spin_once(self.node, timeout_sec=0.1)

        self.assertTrue(self._spin_until(lambda: self.received_nlu is not None, timeout_sec=20.0))
